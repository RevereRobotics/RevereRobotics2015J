...
public class XboxController {
	...
	public double deadband(double JoystickValue, double DeadBandCutOff) {
		double deadBandReturn = (JoystickValue - (Math.abs(JoystickValue) / JoystickValue
				* DeadBandCutOff));

		if (JoystickValue < DeadBandCutOff && JoystickValue > (DeadBandCutOff*(-1))) {
			deadBandReturn = 0;
		} else {
			deadBandReturn = (JoystickValue = (Math.abs(JoystickValue) /
					JoystickValue * DeadBandCutOff)) / (1 - DeadBandCutOff);
		}
